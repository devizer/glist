...
docker exec -t tests bash -c 'source /etc/os-release; echo I am $PRETTY_NAME; echo It is $(uname -m) CPU; apt-get update -qq; apt-get install -y -qq git sudo jq tar bzip2 gzip curl lsb-release procps gnupg2 apt-transport-https dirmngr ca-certificates mc htop nano sudo bash procps mc htop free; swapon'
